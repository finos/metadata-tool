...
(defn- gen-bitergia-person-affiliation
  [affiliation]
  (if-let [organization (md/organization-metadata (:organization-id affiliation))]
    (do
      (println "    - organization:" (:organization-name organization))
      (when-not (s/blank? (:start-date affiliation)) (println "      start:" (:start-date affiliation)))
      (when-not (s/blank? (:end-date affiliation))   (println "      end:"   (:end-date affiliation))))
    (u/println-stderr "⚠️ Warning: unable to find metadata for organization with id" (get affiliation "organizationId"))))


(defn- gen-bitergia-person-entry
  [person]
  (println "\n- profile:")
  (when-not (s/blank? (:full-name            person)) (println "    name:" (:full-name person)))
  (when-not (nil?     (:is-bot               person)) (println "    is_bot:" (:is-bot person)))
  (when     (seq      (:affiliations         person)) (println "  enrollments:") (doall (map gen-bitergia-person-affiliation (:affiliations         person))))
  (when     (seq      (:github-user-ids      person)) (println "  github:")      (doall (map #(println "    -" %)            (:github-user-ids      person))))
  (when     (seq      (:confluence-usernames person)) (println "  confluence:")  (doall (map #(println "    -" %)            (:confluence-usernames person))))
  (when     (seq      (:email-addresses      person)) (println "  email:")       (doall (map #(println "    -" %)            (:email-addresses      person)))))

(defn gen-bitergia-affiliation-data
  []
  (println "# ⚠️ BITERGIA FOLKS: DO NOT EDIT THIS FILE!! ⚠️ ")
  (println "# We generate this file from a separate data source, and your changes WILL BE OVERWRITTEN AND LOST!")
  (println "# If you need to make changes, please create an issue and we'll add your changes to the source data set.")
  (doall (map gen-bitergia-person-entry (md/people-metadata)))
  (println))
